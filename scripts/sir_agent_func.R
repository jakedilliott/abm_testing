...
SIR_agent = function(N         # population size
   ...
   # vstate = 1  means exposed
   # vstate = 2  means infected    
   ...
   vstate[index_inf] = 2     # these are the infected people

   ########################################################################
   # now begin the time steps
   ########################################################################
   t     = tbeg
   S     = S_0  # init susceptible
   E     = E_0  # init exposed
   I     = I_0  # init infective
   R     = R_0  # init recovered
   vS    = numeric(0)  # this will be filled with the number of susceptibles in the population over time
   vE    = numeric(0)  # this will be filled with the number of exposed in the population over time
   vI    = numeric(0)  # this will be filled with the number of infectives in the population over time
   vR    = numeric(0)  # this will be filled with the number of recovred in the population over time
   ...
   while (t<tend&I>0){ # continue the simulation until we have no more infectious people, or t>=tend
      ...
      E = length(vstate[vstate==1])  # count the number of exposed, based on the state vector
      I = length(vstate[vstate==2])  # count the number of infectives, based on the state vector
      R = length(vstate[vstate==3])  # count the number of recovered, based on the state vector
      ...
      vnewstate[vstate==2&vprob<recover_prob] = 3              

      # If a susceptible contacted at least one infective, they are exposed
      vnewstate[vstate==0&vnum_infected_people_contacted>0] = 1
      
      # After a certain number of days without symptoms,
      # exposed become infective
      vnewstate[vstate==1&vprob<De] = 2
   ...
   return(list(time=vtime,S=vS,E=vE,I=vI,R=vR,final_size=final))
